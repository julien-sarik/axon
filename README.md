...
Intercept a message before it is dispatched to a message handler. The interceptor is invoked by the same thread that dispatch the message.
...
to the message.
The `CommandHandlerInterceptor` annotation let you define a handler interceptor on an aggregate's method.
#### exception handler
...
### correlation data
The `CorrelationDataProvider`specifies how Axon should use metadata to correlate messages to their ancestors i.e. the messages responsible for their creation.
Axon provides the `MessageOriginProvider` class that uses:
- a correlation key to correlate a given message to its direct parent
- a trace key to correlate a message to all its ancestors.
